...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = TSNDataSet(
        ...
        return_frame_positions=args.fsn_interaction != 'none')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            selection_score = validate(args, val_loader, model, criterion, epoch, log_training, tf_writer)

            if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                                                        and args.rank % ngpus_per_node == 0):
                # The variable name is retained for checkpoint compatibility.
                is_best = selection_score > best_prec1
                best_prec1 = max(selection_score, best_prec1)
                tf_writer.add_scalar('selection/best_score', best_prec1, epoch)

                output_best = 'Best %s: %.3f\n' % (args.selection_metric, best_prec1)
                print(output_best)
...
def train(args, train_loader, model, scaler, criterion, optimizer, epoch, log, tf_writer):
    ...
    for i, batch in enumerate(train_loader):
        ...
        if args.fsn_interaction != 'none':
            input_glance, images_input, glance_positions, input_positions, target = batch
            glance_positions = glance_positions.cuda(args.gpu, non_blocking=True)
            input_positions = input_positions.cuda(args.gpu, non_blocking=True)
        else:
            input_glance, images_input, target = batch
            glance_positions = input_positions = None
...
def validate(args, val_loader, model, criterion, epoch, log=None, tf_writer=None):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            # compute output
            logit, global_logit, local_logit, temporal_logit, spatial_logit, action, action_3, w, lind = model(
                images_glance=input_glance,
                images_input=images_input,
                glance_positions=glance_positions,
                input_positions=input_positions,
            ...
            if confusion is None:
                confusion = torch.zeros(num_classes, num_classes, device=target.device, dtype=torch.float64)
            predictions = logit.argmax(dim=1)
            bins = torch.bincount(
                target.long() * num_classes + predictions.long(),
                minlength=num_classes * num_classes,
            ).reshape(num_classes, num_classes)
            confusion += bins.to(confusion.dtype)
    ...
    per_class_f1 = 2 * precision * recall / (precision + recall).clamp_min(1e-12)
    macro_f1 = per_class_f1.mean().item() * 100
    weighted_f1 = ((per_class_f1 * support).sum() / support.sum().clamp_min(1)).item() * 100
    micro_f1 = (true_positive.sum() / support.sum().clamp_min(1)).item() * 100

    output = ('Testing Results: Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Macro-F1 {macro_f1:.3f} '
              'Weighted-F1 {weighted_f1:.3f} Loss {loss.avg:.5f}\t'
              ...
              .format(top1=top1_list[0], top5=top5_list[0], macro_f1=macro_f1,
                      weighted_f1=weighted_f1, loss=losses,
    ...
    print(output)
    metrics = {
        'accuracy': top1_list[0].avg,
        'macro_f1': macro_f1,
        'micro_f1': micro_f1,
        'weighted_f1': weighted_f1,
        'per_class_precision': (precision * 100).cpu().tolist(),
        'per_class_recall': (recall * 100).cpu().tolist(),
        'per_class_f1': (per_class_f1 * 100).cpu().tolist(),
        'per_class_support': support.long().cpu().tolist(),
    }
    metrics_line = 'METRICS_JSON ' + json.dumps(metrics, ensure_ascii=False)
    print(metrics_line)
    if log is not None:
        log.write(output + '\n')
        log.write(metrics_line + '\n')
        log.flush()
    ...
    if tf_writer is not None:
        ...
        tf_writer.add_scalar('f1/test_macro', macro_f1, epoch)
        tf_writer.add_scalar('f1/test_weighted', weighted_f1, epoch)

    return macro_f1 if args.selection_metric == 'macro_f1' else top1_list[0].avg
