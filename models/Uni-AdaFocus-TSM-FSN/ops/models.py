...
class TSN(nn.Module):
    ...
    TSN Configurations:
        ...
        if local_adapter_mode not in {'none', 'frame', 'temporal'}:
            raise ValueError("local_adapter_mode must be none, frame, or temporal")
        if local_adapter_mode != 'none':
            if 'resnet' not in base_model:
                raise ValueError("the FSN local adapter currently requires a ResNet local backbone")
            temporal_kernel = 1 if local_adapter_mode == 'frame' else 3
            self.local_adapter = LocalTemporalAdapter(
                channels=1024,
                bottleneck=local_adapter_dim,
    ...
    def _forward_resnet_with_grid(self, frames):
        ...
        The optional FSN adapter is inserted exactly between layer3 and layer4.
        """
        model = self.base_model
        features = model.conv1(frames)
        ...
        feature_grid = model.layer4(features)
        pooled = model.avgpool(feature_grid)
        pooled = torch.flatten(pooled, 1)
        pooled = model.fc(pooled)
        return pooled, feature_grid
        ...
        if self.reshape:
            ...
            if glance:
                return output.squeeze(1), reg_output.squeeze(1), \
                       feature_maps.view((-1, self.num_segments * 1280) + feature_maps.size()[2:]), base_out_feat, reg_out
            elif self.return_feature_grid:
                batch = base_out_feat.shape[0] // self.num_segments
                grid = F.adaptive_avg_pool2d(
                    feature_grid, (self.local_grid_size, self.local_grid_size)
                )
                grid = grid.view(
                    batch, self.num_segments, grid.shape[1], self.local_grid_size, self.local_grid_size
                )
                frame_features = base_out_feat.view(batch, self.num_segments, -1)
                return output.squeeze(1), reg_output.squeeze(1), grid, frame_features
            else:
                return output.squeeze(1), reg_output.squeeze(1)
