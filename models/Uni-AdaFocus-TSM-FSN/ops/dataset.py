...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file, num_segments_glancer=8,
        ...
        self.return_frame_positions = return_frame_positions
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            offsets = np.random.rand(self.num_segments_glancer) + np.arange(self.num_segments_glancer)
            offsets = offsets / self.num_segments_glancer * record.num_frames
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            offsets = np.random.rand(self.num_segments_focuser) + np.arange(self.num_segments_focuser)
            offsets = offsets / self.num_segments_focuser * record.num_frames
            offsets = np.floor(offsets)
    ...
    def _get_val_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            offsets = np.ones(self.num_segments_glancer) * 0.5 + np.arange(self.num_segments_glancer)
            offsets = offsets / self.num_segments_glancer * record.num_frames
            offsets = np.floor(offsets)
            offsets_glancer = offsets + 1
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            offsets = np.ones(self.num_segments_focuser) * 0.5 + np.arange(self.num_segments_focuser)
            offsets = offsets / self.num_segments_focuser * record.num_frames
            offsets = np.floor(offsets)
            offsets_focuser = offsets + 1
    ...
    def get(self, record, indices_glancer, indices_focuser):
        ...
        if self.return_frame_positions:
            denominator = max(record.num_frames - 1, 1)
            glance_positions = (np.asarray(indices_glancer, dtype=np.float32) - 1) / denominator
            focus_positions = (np.asarray(indices_focuser, dtype=np.float32) - 1) / denominator
            glance_positions = np.clip(glance_positions, 0.0, 1.0)
            focus_positions = np.clip(focus_positions, 0.0, 1.0)
            return (
                process_data_glancer,
                process_data_focuser,
                glance_positions,
                focus_positions,
                record.label,
            )
